...
def tableSexRace(df):
    ...
    table = df.groupby(['Race', 'Sex']).size().unstack(fill_value=0)
    ...
    maxGroup = filteredTable.stack().idxmax()
    ...
    print(table)
    print("*" * 35)
    print(f"El grupo más grande es '{maxGroup}' con un {maxPorc:.2f}% del total.")

def tableHourAge(df):
    hourBins = [0, 19, 39, 59, float('inf')]
    hourLabels = ['0-19', '20-39', '40-59', 'Más horas']

    ageBins = [14, 24, 34, 44, 54, 64, 74, 84, 94]
    ageLabels = ['14-24', '25-34', '35-44', '45-54', '55-64', '65-74', '75-84', '85-94']
    ...
    table = df.groupby(['Edad', 'Horas']).size().unstack(fill_value=0)
